...
def createnetasklist():
    os.remove('tasks.txt')
    with open('tasks.txt', 'w') as f:
        f.write('')

def updatetasklist(tasklist):
    os.remove('tasks.txt', 'w')
    with open('tasks.txt', 'w') as f:
...
def remove_task():
    ...
    if task_index < 0 or task_index > len(tasklist):
        return render_template('home.html',datetoday2=datetoday2,tasklist=gettasklist(),l=len(tasklist), mess='Invalid Index....')
    else:
